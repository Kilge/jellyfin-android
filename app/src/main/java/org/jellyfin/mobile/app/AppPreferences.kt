...
class AppPreferences(context: Context) {
    ...
    /**
     * Whether the app should force the web UI to follow the system dark/light theme.
     */
    var followSystemTheme: Boolean
        get() = sharedPreferences.getBoolean(Constants.PREF_FOLLOW_SYSTEM_THEME, true)
        set(value) {
            sharedPreferences.edit {
                putBoolean(Constants.PREF_FOLLOW_SYSTEM_THEME, value)
            }
        }

    /**
     * Last known system dark mode state, updated from the system configuration
     * callback (onConfigurationChanged). The JS bridge reads this instead of
     * querying Resources.getSystem() directly, which can lag on some ROMs.
     */
    var systemDarkTheme: Boolean
        get() = sharedPreferences.getBoolean(Constants.PREF_SYSTEM_DARK_THEME, false)
        set(value) {
            sharedPreferences.edit {
                putBoolean(Constants.PREF_SYSTEM_DARK_THEME, value)
            }
        }
